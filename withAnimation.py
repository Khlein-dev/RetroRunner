...
def bird_animation():
    global bird_index, bird_frames
    bird_index += 0.1  # Increment index by a fraction for smooth animation
    if bird_index >= len(bird_frames):
        bird_index = 0
    return bird_frames[int(bird_index)]

def cat_animation():
    global cat_index, cat_frames
    cat_index += 0.1  # Increment index by a fraction for smooth animation
    if cat_index >= len(cat_frames):
        cat_index = 0
    return cat_frames[int(cat_index)]
...
# Load and scale cat obstacle images (bigger size)
cat1_surface = pygame.image.load('pics/cat.png').convert_alpha()
cat1_surface = pygame.transform.scale(cat1_surface, (70, 70))
cat2_surface = pygame.image.load('pics/cat2.png').convert_alpha()
cat2_surface = pygame.transform.scale(cat2_surface, (70, 70))
cat3_surface = pygame.image.load('pics/cat3.png').convert_alpha()
cat3_surface = pygame.transform.scale(cat3_surface, (70, 70))
cat_frames = [cat1_surface, cat2_surface, cat3_surface]
cat_index = 0

# Load and scale bird obstacle images
bird_surface = pygame.image.load('pics/bird1.png').convert_alpha()
bird_surface2 = pygame.image.load('pics/bird2.png').convert_alpha()
bird_surface3 = pygame.image.load('pics/bird3.png').convert_alpha()
bird_surface = pygame.transform.scale(bird_surface, (70, 70))
bird_surface2 = pygame.transform.scale(bird_surface2, (70, 70))
bird_surface3 = pygame.transform.scale(bird_surface3, (70, 70))
bird_frames = [bird_surface, bird_surface2, bird_surface3]
bird_index = 0

# Initial bird surface (not used directly but for reference)
...
while True:
    for event in pygame.event.get():
        ...
        if game_active:
            ...
            if event.type == obstacle_timer:
                # Randomly spawn cat or bird obstacle
                if randint(0, 1) == 0:
                    # Spawn cat obstacle (start with first frame)
                    obstacle_rect_list.append((cat_frames[0], cat_frames[0].get_rect(midbottom=(randint(900, 1100), 315))))
                else:
                    # Spawn bird obstacle (start with first frame)
                    obstacle_rect_list.append((bird_frames[0], bird_frames[0].get_rect(midbottom=(randint(900, 1100), 200))))
    ...
    if game_active:
        ...
        # Animate birds and cats in obstacle list
        for i, (surface, rect) in enumerate(obstacle_rect_list):
            if surface in bird_frames:
                new_surface = bird_animation()
                obstacle_rect_list[i] = (new_surface, rect)
            elif surface in cat_frames:
                new_surface = cat_animation()
                obstacle_rect_list[i] = (new_surface, rect)
